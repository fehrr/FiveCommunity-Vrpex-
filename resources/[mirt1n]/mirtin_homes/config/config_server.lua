...
config.lotus = true
config.iptuValue = 0.18-- 7% preço da propriedade
...
-- COMANDOS
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand('euvotacriandoessacasaaqui7282849492@284848429293949949', function(source,args) -- Comando criarcasa
    local user_id = vRP.getUserId(source)
    if user_id then
        if vRP.hasPermission(user_id, "developer.permissao") then -- permissao para criar a casa
...
RegisterCommand('euvotacriandoessagaragemaqui7282849492@284848429293949949', function(source,args) -- Comando deletarcasa
    local user_id = vRP.getUserId(source)
    if user_id then
        
        if vRP.hasPermission(user_id, "developer.permissao") then -- permissao para deletar a casa
            local idProp = vRP.prompt(source, "Digite o ID da propriedade: ", "")
            if idProp ~= nil and tonumber(idProp) then
                if src.checkIdHouse(idProp) then
...
RegisterCommand('home', function(source,args) -- Comando gerais das casas
    ...
    if user_id then
        ...
        if args[1] == "localizar" and tonumber(args[2]) then
            if vRP.hasPermission(user_id, config.permissionLocalizar) or vRP.hasPermission(user_id, config.permissionLocalizar2) or vRP.hasPermission(user_id, "developer.permissao") or vRP.hasPermission(user_id, "perm.respilegal") then
        ...
        if args[1] == "info" and tonumber(args[2]) then
            if not vRP.hasPermission(user_id, "perm.imobiliaria") and not vRP.hasPermission(user_id, "developer.permissao") and not vRP.hasPermission(user_id, "diretor.permissao") then
                TriggerClientEvent("Notify",source,"negado","Você não possui permissão.", 5000)
                return
            end
            local owner2 = vRP.query("mirtin/ownerPropriedade", { houseID = tonumber(args[2]) })
        ...
        if args[1] == "ver" and tonumber(args[2]) then
            if vRP.hasPermission(user_id, config.permissionLocalizar) or vRP.hasPermission(user_id, "perm.respilegal") or vRP.hasPermission(user_id, "developer.permissao") or vRP.hasPermission(user_id, config.permissionLocalizar2) then
        ...
        if args[1] == "vender" and tonumber(args[2]) then
            ...
            vRP.sendLog("https://discord.com/api/webhooks/1120030715694821407/eY9t8SHGnb4fkvqVjNxjF0Rbq59JdDlC6wHBfzxXbZVD7OmXE7UcnYxnXIacabdNBKfc", "```prolog\n[USER_ID]: "..user_id.."\n[VENDEU A CASA]: "..tonumber(args[2]).."\n[VALOR]: "..value.."\n[PARA]: "..nuser_id.."```")
