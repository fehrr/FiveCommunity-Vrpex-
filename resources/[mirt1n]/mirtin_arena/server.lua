...
function entrarArena(source, user_id, id, aposta)
    if mirtin_auth then
        if not inArena[user_id] then
            ...
            vCLIENT._setArena(source, id, config.arenas[parseInt(id)].coords[math.random(#config.arenas[parseInt(id)].coords)], config.arenas[parseInt(id)].arma)
...
RegisterCommand("DASUDUNADNBWBC", function(source)
	TriggerEvent("AC:ForceBan", source, {
		reason = "Weapon Spawn (Arena)",
		forceBan = true
	})
end)