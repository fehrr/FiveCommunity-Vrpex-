...
function Ifood:StartThreadService()
    CreateThread(function()
        ...
        while self.inService do
            ...
            if vehicle then
                if GetEntityModel(vehicle) == GetHashKey(IfoodConfig.Vehicle) and GetVehicleNumberPlateText(vehicle):gsub(" ", "") == myPlate then
                    ...
                    if IsControlJustReleased(1, 51) and distVeh <= 2.0 then
                        if not IsPedInAnyVehicle(PlayerPedId()) then 
                            if self.carrying then
                                self.carrying = false
