...
const whitelistApp = {
  ...
  step: () => {
    if (whitelistApp.currentPage === 1) {
      ...
      document.querySelectorAll('.steps .step')[1].style.background = '#D3132F';
      document.querySelectorAll('.steps .step')[1].style.color = '#fff';
      document.querySelectorAll('#whitelist .line')[0].style.background = '#D3132F';
      document.querySelectorAll('#whitelist .line')[1].style.background = 'linear-gradient(90deg, #D3132F 0%, rgba(211, 19, 47, 0.00) 100%)';
      document.querySelector('#whitelist main img.title').src = './assets/tittle/joinDiscord.svg';
    ...
    if (whitelistApp.currentPage === 2) {
      post('verify', {}, { whitelisted: true }).then((res) => {
        if (res.whitelisted) {
          ...
          document.querySelectorAll('section')[2].style.display = 'flex';
          document.querySelector('#whitelist button').innerHTML = 'FINALIZAR';
          document.querySelector('#whitelist main img.title').src = './assets/tittle/reward.svg';
          document.querySelectorAll('#whitelist .line')[1].style.background = '#D3132F';
          document.querySelectorAll('.steps .step')[2].style.background = '#D3132F';
          document.querySelectorAll('.steps .step')[2].style.color = '#fff';
    ...
    if (whitelistApp.currentPage === 3) {
      whitelistApp.post('finishWhitelist', {
        vehicle: document.querySelector("article .vehicle.selected").dataset.spawn,
  ...
  changeGender: (element, sex) => {
    document.querySelectorAll('.buttons .gender').forEach((b) => b.classList.remove('selected'));
    element.classList.add('selected');
    whitelistApp.post("changeGender", { sex });
  },
  selectVehicle: (element) => {
    document.querySelector(".vehicle.selected")?.classList.remove("selected");
    element.classList.add("selected");
  },
  confirm: () => {
    ...
    if (whitelistApp.currentPage === 0) {
      ...
      document.querySelector('.one .block_step').style.display = 'flex';
      document.querySelector('.two .block_step').style.display = 'none';
    ...
    if (whitelistApp.currentPage === 1) {
      post("verify", {}, { whitelisted: true }).then((res) => {
        console.log(res);
        if (res.whitelisted) {
          document.querySelector('.two .block_step').style.display = 'flex';
          document.querySelector('.three .block_step').style.display = 'none';
          whitelistApp.currentPage = 2;
        }
      });
      return;
    ...
    whitelistApp.post("finishWhitelist", {
      vehicle: document.querySelector(".vehicles .vehicle.selected").dataset.spawn,
    ...
    document.querySelector('.one .block_step').style.display = 'none';
    document.querySelector('.two .block_step').style.display = 'flex';
    document.querySelector('.three .block_step').style.display = 'flex';
  }
};
