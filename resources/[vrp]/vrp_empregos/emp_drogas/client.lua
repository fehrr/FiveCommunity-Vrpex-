...
Citizen.CreateThread(function()
    while true do
        ...
        if servico then
            time = 5
            -- Verifica o modo e desenha o texto correspondente
            if mode > 1 then
                drawTxt("~r~ ~w~Aperte ~r~F7~w~ se deseja finalizar o expediente.", 0.215, 0.94)
            else
                drawTxt("~g~ ~w~Aperte ~r~F7~w~ se deseja finalizar o expediente..", 0.215, 0.94)
            end

            -- Controle para finalizar o expediente
...
Citizen.CreateThread(function()
    while true do
        ...
        -- Verifica o estado do serviço e cria uma nova missão para entregar as drogas
...
Citizen.CreateThread(function()
    while true do
        local time = 1000
        -- Atualiza o cooldown de segundos
        if segundos >= 0 then
            segundos = segundos - 1
